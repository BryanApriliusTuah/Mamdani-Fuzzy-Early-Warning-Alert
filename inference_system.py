...
class CleanFuzzyVisualizer:
	...
	def visualize_all_steps(self, water_val, rate_val, rain_val):
		...
		#Step 3: Combined Clipping
		fig3 = self.plot_combined_imp(water_val, rate_val, rain_val)
		fig3.savefig('combined_clipping.png')
	...
	def plot_imp(self, water_val, rate_val, rain_val):
		"""Plot implication - clipping of membership functions for individual plots"""
		...
		for term in active_water.keys():
			...
			ax.plot(self.water_level, np.fmin(self.water_mf[term], water_mem[term]), 
					label=f'{term} (μ={individual_membership:.2f})',
					linewidth=2.5, color=self.color_water[term])
			ax.fill_between(self.water_level, 0, np.fmin(self.water_mf[term], water_mem[term]), 
							alpha=0.2, color=self.color_water[term])
		ax.set_title(f'Water Level Clipping (clipped at μ={water_mem[term]:.2f})', fontweight='bold', fontsize=12)
		ax.set_xlabel('Normalized Value')
		ax.set_ylabel('Membership Degree (μ)')
		ax.legend(fontsize=9)
		ax.grid(True, alpha=0.3)
		...
		for term in active_rate.keys():
			...
			ax.plot(self.rate_change, np.fmin(self.rate_mf[term], rate_mem[term]), 
					label=f'{term} (μ={individual_membership:.2f})',
					linewidth=2.5, color=self.color_rate[term])
			ax.fill_between(self.rate_change, 0, np.fmin(self.rate_mf[term], rate_mem[term]), 
							alpha=0.2, color=self.color_rate[term])
		ax.set_title(f'Rate of Change Clipping (clipped at μ={rate_mem[term]:.2f})', fontweight='bold', fontsize=12)
		ax.set_xlabel('Normalized Value')
		ax.set_ylabel('Membership Degree (μ)')
		ax.legend(fontsize=9)
		ax.grid(True, alpha=0.3)
		...
		rain_representative = min(active_rain.values()) if active_rain else 0
		# Plot Rainfall - ALL clipped at min_clip
		ax = axes[2]
		for term in active_rain.keys():
			individual_membership = rain_mem[term]
			ax.plot(self.rainfall, np.fmin(self.rain_mf[term], rain_representative), 
					label=f'{term} (μ={individual_membership:.2f})',
					linewidth=2.5, color=self.color_rain[term])
			ax.fill_between(self.rainfall, 0, np.fmin(self.rain_mf[term], rain_representative), 
							alpha=0.2, color=self.color_rain[term])
		ax.set_title(f'Rainfall Clipping (clipped at μ={rain_representative:.2f})', fontweight='bold', fontsize=12)
		ax.set_xlabel('Normalized Value')
		ax.set_ylabel('Membership Degree (μ)')
		ax.legend(fontsize=9)
		ax.grid(True, alpha=0.3)
	...
	def plot_combined_imp(self, water_val, rate_val, rain_val):
		"""Plot combined implication - all clipped inputs in one plot"""
		# Calculate memberships
		water_mem = self.calculate_memberships(self.water_level, self.water_mf, water_val)
		rate_mem = self.calculate_memberships(self.rate_change, self.rate_mf, rate_val)
		rain_mem = self.calculate_memberships(self.rainfall, self.rain_mf, rain_val)
		...
		for term in active_rate.keys():
			...
			ax.fill_between(rate_scaled_x, 0, np.fmin(self.rate_mf[term], rate_mem[term]), 
							alpha=1, color='#000000')
